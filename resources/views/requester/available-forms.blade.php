                    ...
                    @foreach($templates as $template)
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                            ...
                            @if($template->description)
                                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{{ Str::limit($template->description, 100) }}</p>
                            @endif
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-500">{{ $template->input_field_templates_count }} fields</p>
                            <a href="{{ route('requester.forms.create', $template) }}" class="mt-4 inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                                Start Form
                            </a>
                        </div>
                    @endforeach
